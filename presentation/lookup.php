<?php
  ...
  function ajax_lookup($metabasename, $databasename, $fieldname, $value, $presentationname, $foreigntablename, $foreigntablenamesingular, $foreignuniquefieldname, $nullallowed, $defaultvalue, $readonly, $extra = true) {
    ...
    $references = query("SELECT `$foreignviewname`.`$foreignuniquefieldname` AS `_id`, $descriptor[select] AS `_descriptor` FROM `$databasename`.`$foreignviewname` ".join(' ', $descriptor['joins']).($readonly ? ($value ? "WHERE `$foreignviewname`.`$foreignuniquefieldname` = \"".((int) $value)."\"" : "LIMIT 0") : "ORDER BY ".join(', ', $descriptor['orders'])), $descriptor['arguments']);
