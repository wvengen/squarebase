<?php
  function probability_boolean($field) {
    if (preg_match('@^(bit|bool|boolean|tinyint)\b@', $field['column_type']))
      return 0.5;
    $distinct = query1("SELECT COUNT(`<fieldname>`) AS numberofrows, SUM(IF(`<fieldname>` = 0, 1, 0)) AS numberofzeros, SUM(IF(`<fieldname>` != 0, 1, 0)) AS numberofnonzeros, COUNT(DISTINCT(`<fieldname>`)) AS numberofdistinctvalues FROM `<databasename>`.`<tablename>`", array('databasename'=>$field['table_schema'], 'tablename'=>$field['table_name'], 'fieldname'=>$field['column_name']));
