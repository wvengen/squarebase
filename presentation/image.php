<?php
  ...
  function formfield_image($metabasename, $databasename, $field, $value, $readonly) {
    return
      $readonly
      ? cell_image($metabasename, $databasename, $field, $value)
      : html('ul', array('class'=>join(' ', cleanlist(array($field['presentation'], $readonly ? 'readonly' : null, $field['nullallowed'] ? null : 'notempty')))),
          html('li', array(),
            array(
              $value ? html('input', array('type'=>'radio', 'class'=>'radio', 'name'=>"radio:$field[fieldname]", 'id'=>"radio:original:$field[fieldname]", 'value'=>'original', 'checked'=>'checked'), html('input', array('type'=>'hidden', 'name'=>"original:$field[fieldname]", 'value'=>base64_encode($value))).html('label', array('for'=>"radio:original:$field[fieldname]"), cell_image($metabasename, $databasename, $field, $value))) : null,
  ...
  function cell_image($metabasename, $databasename, $field, $value) {
